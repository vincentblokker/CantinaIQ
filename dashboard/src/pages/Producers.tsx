  ...
  return (
    <div>
      <div className="flex items-baseline gap-3 mb-2 flex-wrap">
        <h2 className="font-serif text-2xl text-ink">{t("producers.title")}</h2>
        <p className="text-xs text-ink-2 italic ml-2">
          {t("producers.hintPre")} <span className="inline-flex w-4 h-4 rounded-full border border-tuscan/40 text-tuscan items-center justify-center text-[10px] font-bold align-middle">i</span> {t("producers.hintPost")}
        </p>
      </div>
      ...
      <div className="flex items-baseline gap-3 flex-wrap mb-4">
        ...
        <span className="ml-auto text-xs text-ink-2 tabular-nums">
          {filtered.length === rows.length
            ? t("producers.countAll", { producers: rows.length })
            : t("producers.countFiltered", { shown: filtered.length, total: rows.length })}
        </span>
      </div>
      ...
      <table className="w-full text-sm border border-stone-200 rounded-lg overflow-hidden bg-white">
        <thead className="bg-stone-50 text-ink-2 text-xs uppercase tracking-wide">
          <tr>
            <th className="text-left px-3 py-2">{t("producers.colProducer")}</th>
            <th className="text-left px-3 py-2">{t("producers.colMacroRegion")}</th>
            <th className="text-left px-3 py-2">{t("producers.colRecommendation")}</th>
            <th className="text-right px-3 py-2">{t("producers.colWeightedRating")}</th>
            <th className="text-right px-3 py-2">{t("producers.colAvgPrice")}</th>
            <th className="text-right px-3 py-2">{t("producers.colComposite")}</th>
            ...
            >
              ...
              <td className="px-3 py-2 text-right">
                <button
                  ...
                  aria-label={t("producers.moreInfo", { name: p.producer_name })}
